...
pub use j::{
    ...
    CALYX_ANNEAL_J_INVALID_CONFIG, CALYX_ANNEAL_J_INVALID_METRIC, CALYX_ANNEAL_REPORT_INVALID_ROW,
    CandidateAction, DEFAULT_CROSS_LENS_DOMINANCE_THRESHOLD,
    DEFAULT_GOODHART_VIOLATION_PENALTY_WEIGHT, DEFAULT_GROWTH_MAX_SAMPLES, DEFAULT_GROWTH_WINDOW,
    DEFAULT_GTAU_THRESHOLD, DEFAULT_HELD_OUT_MIN_GAIN_FRACTION, DEFAULT_J_DOMAIN, GoodhartChecker,
    GoodhartLedgerContext, GoodhartReport, GoodhartState, GoodhartViolation, GradientCandidate,
    GradientEntry, GradientEntryReadback, GradientRefreshReport, GradientSnapshot, GradientWarning,
    GrowthCf, GrowthCurve, GrowthSample, GrowthSummary, HeldOutSet, IntelligenceGradient,
    IntelligenceReport, JMetricSources, JObjectiveContext, JTermDeltas, JTerms, JValue, JWeights,
    LensContributionDelta, PriorityReadback, REDUNDANCY_PENALTY, ReportAvailability, ReportDiff,
    TuneScopeKind, UNIT_PENALTY, WardGtau, add_goodhart_penalty_to_vault, anneal_growth_key,
    anneal_report_key, compute_j, decode_growth_row, decode_intelligence_report_row,
    encode_growth_row, estimate_dj, format_report, goodhart_state_path, gradient_state_path,
    intelligence_report, j_weights_path, latest_intelligence_report_snapshot,
    read_goodhart_state_from_vault, read_gradient_snapshot_from_vault,
    read_intelligence_report_snapshot, read_objective_weights_from_vault, record_goodhart_report,
    report_diff, set_objective_weights, to_json, write_goodhart_state, write_gradient_snapshot,
...
pub use propose::{
    AdmissionRecord, AlgParams, AlgorithmicKind, AnchorGap, AnchorId, AssayAttribution,
    CALYX_ANNEAL_CANDIDATE_INVALID_DEFICIT, CALYX_ANNEAL_DEFICIT_INVALID_CONFIG,
    CALYX_ASSAY_INVALID_METRIC, CALYX_ASSAY_UNAVAILABLE, CALYX_REGISTRY_HOT_ADD_FAIL,
    CALYX_REGISTRY_PROFILE_TIMEOUT, CandidateLens, CommissionSpec, CorpusSampleSource,
    DEFAULT_DEFICIT_THRESHOLD_BITS, DIFFERENTIATION_MAX_CORR, DIFFERENTIATION_MIN_BITS,
    DeficitLocalizer, DeficitLocalizerConfig, DeficitMap, DifferentiationGate, GateOutcome,
    HotAddAction, HotAddPlan, HotAddReceipt, LensAdmittedEntry, LensHotAdder, LensProfiler,
    LensRejectedEntry, MAX_SYNTHESIS_CORPUS_SAMPLE, MODALITY_COVERAGE_THRESHOLD_BITS, ModalityId,
    PROFILE_TIMEOUT_MS, PairNMI, ProposalHistoryReadback, ProposalOutcome, ProposalSubstrate,
    ProposalTerminalState, ProposeLens, ProposeLensRequest, RegistryHotAdder, RejectReason,
    build_commission_spec, describe, describe_gate_outcome, gate, has_deficit, proposal_history,
    proposal_history_with_refs, propose_lens, record_admitted, record_from_entry,
...
pub use tune::{
    ...
    ConcatKey, ConfigBandit, ConfigBanditStore, ConfigVariant, DEFAULT_AB_MIN_SAMPLES,
    DEFAULT_FORGE_RECALL_TARGET, DEFAULT_HYSTERESIS_WINS, DEFAULT_INDEX_RECALL_TARGET,
    DEFAULT_INDEX_VRAM_BUDGET_BYTES, DEFAULT_LOOM_RECALL_TARGET, DEFAULT_SOAK_OSCILLATION_WINDOW,
    DEFAULT_SOAK_P99_TARGET_REDUCTION, DEFAULT_SOAK_QUERIES, DEFAULT_SOAK_SAMPLE_INTERVAL,
    DEFAULT_SOAK_SEED, DType, ForgeBanditPersistence, ForgeConfig, ForgePromotionRecord,
    ForgePromotionWriter, ForgeScopeTuner, ForgeTuneDecision, IndexBanditPersistence, IndexConfig,
    IndexPromotionRecord, IndexPromotionWriter, IndexScopeTuner, IndexSlotHealth,
    IndexTuneDecision, IndexTuneSkip, LoomBanditPersistence, LoomMaterializer, LoomPromotionRecord,
    LoomPromotionWriter, LoomScopeTuner, LoomTuneDecision, MAX_BUCKETED_DIM, MAX_FORGE_CANDIDATES,
    MAX_INDEX_CANDIDATES, MAX_LOOM_CANDIDATES, MAX_LOOM_EAGER_PAIRS, MIN_BITS_PER_ANCHOR,
