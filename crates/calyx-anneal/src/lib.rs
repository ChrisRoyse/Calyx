...
pub use learn::{
    ...
    CALYX_ANNEAL_MISTAKE_INVALID_ROW, CALYX_ANNEAL_REPLAY_INVALID_ROW, CALYX_REGISTRY_UNAVAILABLE,
    DEFAULT_MISTAKE_SURPRISE_THRESHOLD, DEFAULT_REPLAY_CAPACITY, FrozenCheckReport,
    FrozenLensCheck, FrozenLensGuard, FrozenLensReportRow, FrozenLensSource, FrozenLensStatus,
    HeadKind, HeadPromotionGate, HeadReadback, HeadShadowProposal, HeadStorage, HeadUpdateOutcome,
    HeadUpdateSummary, MAX_ONLINE_HEAD_PARAMS, MistakeEntry, MistakeLog, MistakeReadback,
    MistakeRef, MistakeStorage, NoFrozenLensGuard, OnlineHead, OnlineHeadState, ReplayBuffer,
    ReplayEntry, ReplaySnapshot, ReplayStorage, decode_head_rows, decode_mistake_entry,
    decode_online_head, decode_replay_snapshot, encode_mistake_entry, encode_online_head,
    encode_replay_snapshot, head_key, head_state_artifact_key, mistake_key, mistake_seq_from_key,
    replay_snapshot_key,
};
