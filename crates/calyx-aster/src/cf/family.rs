...
impl ColumnFamily {
    ...
    /// [`Self::STATIC`] (0..27), which stays in sync automatically if the
    /// manifest order is extended. Slot CFs encode to
    /// `SLOT_TAG ‖ slot_id_be(2) ‖ kind_byte` so the slot index and
    /// quantized/raw flavor round-trip exactly. `STATIC` has 27 entries, so no
...
/// discriminant because `STATIC.len()` (27) is far below this value.
