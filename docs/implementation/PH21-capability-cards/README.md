...
flagged automatically. Signal and redundancy delegate to Assay (Stage 5) once
it exists; until then, spread/cost/coverage are standalone.
...
| File | Responsibility |
|---|---|
| `crates/calyx-registry/src/profile.rs` | `CapabilityCard`, `ProbeSet`, `profile()`, spread metrics, silhouette, cost measurement |
...
- **Signal stub:** `signal` and `differentiation` fields must not be silently
  zero-filled — they must carry a `Option<f32>` with `None` until Assay is
  wired, so callers know they are uncomputed. Document this explicitly.
