...
`calyx-sextant` has per-slot HNSW (PH23) and inverted index (PH25). The index
dispatch trait (`SlotIndex`) selects between `HnswIndex` and `InvertedIndex` at
the slot level. No disk-resident graph code exists; `index/diskann` and
`index/spann` are greenfield. The vault physical layout (`04 §3`) already reserves
`idx/slot_NN.ann/` for DiskANN and `idx/slot_NN.sparse/` for SPANN — these paths
are pre-allocated but empty until this phase.
...
5. All byte-level evidence (file sizes, p99 measurement, autotune log) attached to
