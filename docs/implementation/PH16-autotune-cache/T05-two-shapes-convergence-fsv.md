...
- [ ] `tests/autotune_tests.rs`: test `autotune_two_shapes_converge` — marked
  `#[cfg_attr(not(feature="cuda"), ignore)]`
- [ ] Build `candidate_pool_A` for shape `[512,512,512]`: 4 `BestConfig` variants
  differing in `tile_m` (32, 64, 128, 256) but same `backend=Cuda`
- [ ] Build `candidate_pool_B` for shape `[128,768,64]`: 4 variants with different
  tile sizes appropriate for the narrow shape
- [ ] Run explorer for 20 iterations per shape on aiwonder:
  ```
  for iter in 0..20 {
  ...
  ```
- [ ] After the loop: `autotune(&cache, &key_a)` and `autotune(&cache, &key_b)` →
  assert both return `Some` config; assert the two configs differ (at least one field differs)
- [ ] Print: `config_a=tile_m={X}`, `config_b=tile_m={Y}`, `X != Y` assertion
- [ ] Write the final cache to `tests/autotune_cache_fsv.json` (persisted file for FSV readback)
- [ ] Test `autotune_promotion_logged`: after running the above, read the `promotion_log.jsonl`
  and assert it contains at least one `Promoted` event
- [ ] Test `autotune_promotion_reversible`: pick the last promoted key; call `rollback_promotion`;
  assert cache returns old config; assert log now has a `RolledBack` event

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] `autotune_two_shapes_converge`: asserts two distinct keys in cache after 20 iters;
  configs differ; prints both configs
- [ ] `autotune_promotion_logged`: asserts ≥1 `Promoted` event in log
- [ ] `autotune_promotion_reversible`: rollback succeeds; log has `RolledBack`; cache = old config
- [ ] proptest (CPU-only, no `#[cfg(feature="cuda")]`): `AutotuneCache` with 100 random
  `(key, config)` inserts → `get` returns the right config for all 100 keys (no hash collision)
- [ ] edge (≥3): (1) both shapes converge to same tile_m by chance — test passes (distinctness
...
- [ ] CPU↔GPU bit-parity ≤ 1e-3 on the golden set (autotune ops measured in PH12–PH15)
- [ ] **Two shapes converge to two cached configs** (`autotune_two_shapes_converge` is the proof)
- [ ] **Promotion is logged + reversible** (`autotune_promotion_logged` + `autotune_promotion_reversible`)
- [ ] FSV evidence (`/tmp/ph16_fsv.txt` + `autotune_cache_fsv.json`) attached to PH16 GitHub issue
