...
- [ ] `src/quant/mod.rs`: `pub enum QuantLevel { F32, Bits8, Bits3p5, Bits2p5, Bits1 }`
...
- [ ] `pub trait Quantizer: Send + Sync` with methods:
  ...
  `fn dim(&self) -> usize`
- [ ] `pub struct QuantizedVec { pub level: QuantLevel, pub dim: usize, pub bytes: Vec<u8>, pub scale: f32, pub seed_id: SeedId }`
  — `SeedId` is a `[u8; 32]` (SHA-256 content-address of the rotation seed);
  `serde`, `Clone`, `Debug`
- [ ] Extend `ForgeError` (in `src/error.rs`) with:
...
- [ ] Re-export `Quantizer`, `QuantLevel`, `QuantizedVec`, `SeedId` from `lib.rs`

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: `QuantLevel::Bits3p5.bits_per_channel()` == 3.5; `Bits1.is_lossy()` == true
- [ ] unit: `QuantizedVec` serde round-trip: serialize to JSON and back → equal
- [ ] unit: `QuantLevel` all variants round-trip through `serde_json`
- [ ] proptest: any `ForgeError::QuantError` Display starts with `"CALYX_FORGE_QUANT_ERROR"`
- [ ] edge (≥3): (1) `F32.is_lossy()` == false; (2) `Bits2p5.bits_per_channel()` == 2.5;
  (3) `QuantizedVec { bytes: vec![], .. }` serializes without panic
- [ ] fail-closed: constructing a `ForgeError::SeedVersionMismatch { expected: 1, got: 2 }`
