...
single-lens + Δ (≥15%) on real qrels (BEIR/MS MARCO subset on aiwonder) plus
every Hit carrying a real LedgerRef (`14 §2`, `19 §4`).
...
| `crates/calyx-sextant/tests/fusion_recall.rs` | multi-lens vs single-lens recall on real qrels |
...
| T07 | Multi-lens recall FSV on real qrels (BEIR/MS MARCO) | T06 |

## FSV exit gate (the phase is DONE only when this is byte-proven on aiwonder)

Run `cargo test -p calyx-sextant fusion_recall -- --nocapture --ignored` on
aiwonder against the BEIR/MS MARCO qrels subset. The output must print
...
set and confirm `hit.provenance` is a valid (non-zero) `LedgerRef`. Both
printed values + the `Hit` provenance hex are attached to the PH24 GitHub issue.
...
- **LedgerRef stub**: until PH35 ships, use a `LedgerRef::stub(cx_id, seq)` that
  ...
  serialization.
- **qrels dataset**: BEIR/MS MARCO subset must be on aiwonder before the FSV run;
