...
- [ ] `Bm25Config` struct: `k1: f32 = 1.2, b: f32 = 0.75` with `Default` impl
- [ ] `fn bm25_score(tf: u32, df: u32, n: u32, avgdl: f32, dl: u32, cfg: &Bm25Config) -> f32`:
      ```
...
- [ ] `fn score_query(index: &InvertedIndex, query_tokens: &[String], cfg: &Bm25Config) -> HashMap<u32, f32>`:
      for each query token, iterate its postings, accumulate `bm25_score` per doc_id
- [ ] Wire into `InvertedIndex::search`: call `score_query`, sort by score desc,
      map internal doc_ids back to `CxId`, return top-k
- [ ] `CALYX_SEXTANT_BM25_ZERO_DOCS`: if `index.total_docs == 0`, fail-closed

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: hand-crafted corpus (3 docs, known TF/DF):
      ```
...
- [ ] unit: `bm25_score(tf=1, df=1, n=1, avgdl=5.0, dl=5, cfg=default)` →
      known value (compute: idf=ln(1.5), tf_norm=2.2/2.2=1.0, score=ln(1.5)≈0.405);
      assert within 1e-4
- [ ] unit: increasing tf → increasing score (monotone in TF for fixed DF)
- [ ] unit: increasing df → decreasing score (monotone in DF for fixed TF)
- [ ] proptest: `bm25_score` is non-negative for all valid non-zero inputs
- [ ] edge: `df == 0` → treat as 1 (defensive; IDF denominator never 0)
- [ ] edge: `total_docs == 0` → `CALYX_SEXTANT_BM25_ZERO_DOCS`
- [ ] fail-closed: NaN in score → `CALYX_SEXTANT_BM25_NAN` (check after computation;
