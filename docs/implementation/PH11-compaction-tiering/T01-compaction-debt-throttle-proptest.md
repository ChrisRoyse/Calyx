...
- [ ] Add proptest: for any `(pending_bytes, target_bytes > 0)`,
  `CompactionDebt::measure(...).score_milli == pending_bytes * 1000 / target_bytes`
  (integer division, no overflow for sane inputs).
- [ ] Add test: `CompactionDebt { pending=0, target=64MiB }.score_milli == 0`.
- [ ] Add test: `CompactionDebt { pending=64MiB, target=64MiB }.score_milli == 1000`.
- [ ] Add test: `CompactionDebt { pending=128MiB, target=64MiB }.score_milli == 2000`.
- [ ] Add test: `compact_shards` with `throttle.max_input_bytes = Some(32 MiB)`
  and `pending_bytes = 64 MiB` → `CompactionResult::Skipped { debt }` where
  `debt.pending_bytes == 64 MiB`.
- [ ] Add proptest: for any `shards` list, `CompactionDebt::measure(shards).pending_bytes
  == shards.iter().map(|s| s.bytes).sum()`.
- [ ] Add test: `compact_shards` on an empty input list → `Skipped` (no crash,
  no SST output file).

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: 3 known-size shards → debt.pending_bytes is sum of sizes.
- [ ] unit: throttle skips when pending > max_input.
- [ ] proptest: `score_milli` formula correct for all (pending, target) pairs.
- [ ] edge (≥3): (1) target=0 → `target_bytes.max(1)` prevents div-by-zero;
  (2) single shard → compacted to 1 output; (3) 0 shards → Skipped.
- [ ] fail-closed: `compact_shards` on a shard with a non-existent path →
