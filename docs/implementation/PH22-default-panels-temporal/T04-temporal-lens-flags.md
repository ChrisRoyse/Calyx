...
- [ ] Update all existing unit tests that construct `LensSpec` to add the new
  default fields (they default to `false`).

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: `E2RecencyLens` spec has `retrieval_only = true` and
  `excluded_from_dedup = true` after construction.
- [ ] unit: `E3PeriodicLens` and `E4PositionalLens` same.
- [ ] unit: a non-temporal `AlgorithmicLens` spec has both flags `false`.
- [ ] unit: `LensSpec` serde round-trip with `retrieval_only = true` produces
  correct JSON `"retrieval_only": true`.
- [ ] unit: `LensSpec` with old JSON (no `retrieval_only` key) deserializes
  with `retrieval_only = false` (serde default).
- [ ] edge (≥3): (1) `TEMPORAL_FLAGS` constant has both fields true; (2)
  ...
  debug builds.
- [ ] fail-closed: N/A — flags are metadata; no runtime errors here.
