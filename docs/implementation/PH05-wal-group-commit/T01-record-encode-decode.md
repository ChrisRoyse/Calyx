...
- [ ] Add `proptest` dev-dependency to `calyx-aster/Cargo.toml` if not present.
- [ ] Write `proptest!` macro test: for any `(seq: u64, payload: Vec<u8>)` with
  `payload.len() <= 64 * 1024 * 1024`, assert
  `decode_at(encode(seq, payload)) == Complete(DecodedRecord { seq, payload, .. })`.
- [ ] Write deterministic golden test: `encode(42, b"hello")` produces the exact
...
- [ ] Edge case: zero-length payload encodes and round-trips.
- [ ] Edge case: payload length = `MAX_RECORD_BYTES` (64 MiB) encodes without
  truncation; payload length = `MAX_RECORD_BYTES + 1` returns
  `io::Error(InvalidInput)`.
- [ ] Fail-closed: flip one bit in the CRC field of an encoded record; assert
  `decode_at` returns `Torn { offset: 0, message: contains("crc mismatch") }`.
- [ ] Fail-closed: truncate encoded bytes to `HEADER_LEN - 1`; assert
  `Torn { message: contains("partial WAL header") }`.
- [ ] Fail-closed: truncate encoded bytes to exactly the header with zero payload
  bytes when `len > 0`; assert `Torn { message: contains("partial WAL payload") }`.
- [ ] Fail-closed: write magic bytes `0x00000000`; assert
  `Torn { message: contains("bad WAL magic") }`.

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: `encode(42, b"hello")` → byte-exact golden (compute CRC in test with
  `crc32fast::Hasher`, assert `output[0..4] == *b"CXW1"` and `output[4..12] ==
  42u64.to_le_bytes()`).
- [ ] proptest: `decode_at(encode(seq, payload)) == Complete(DecodedRecord { seq,
  payload })` for all `(seq: u64, payload in 0..=1024 bytes)`.
- [ ] edge (≥3): (1) zero-byte payload round-trips; (2) max payload length
  accepted; (3) max+1 payload rejected with `InvalidInput`; (4) partial header
  → `Torn`.
- [ ] fail-closed: single-bit CRC flip → `Torn` with message containing
