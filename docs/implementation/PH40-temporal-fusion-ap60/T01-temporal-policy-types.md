...
- [x] Define `BoostConfig { causal_high_mult: f32, causal_low_mult: f32 }` with defaults `1.10` / `0.85`
...
- [x] edge: `BoostConfig { causal_high_mult: 0.0, .. }` allowed (no range constraint on mult itself — gate is semantic, not structural)
...
- [x] FSV evidence (readback output / screenshot) attached to the PH40 GitHub issue
