...
- [ ] Implement `partitioned_histogram_nmi_v1(x_scalars: impl Iterator<Item=f32>, y_scalars: impl Iterator<Item=f32>, n_bins: usize, n_samples_hint: usize) -> Result<NmiEstimate, CalyxError>`:
...
- [ ] Implement `auto_n_bins(n: usize) -> usize`: `max(5, (n as f64 / 5.0).sqrt().floor() as usize)`
- [ ] Pair-redundancy fast path `pair_redundancy_nmi(slot_a: SlotId, slot_b: SlotId, vault, clock) -> Result<NmiEstimate, CalyxError>`: reads the Agreement scalar stream from the xterm CF (fast: scalars not full vectors), calls `partitioned_histogram_nmi_v1` on the scalar streams
