...
- [ ] `src/cpu/guard.rs`: `pub fn check_finite(slice: &[f32], op: &str) -> Result<(), ForgeError>`
...
- [ ] `pub fn check_norm_positive(norm: f32, op: &str, row: usize) -> Result<(), ForgeError>`
...
- [ ] `pub fn check_shape_2d(slice: &[f32], rows: usize, cols: usize, name: &str) -> Result<(), ForgeError>`
  — `slice.len() == rows * cols` or `ForgeError::ShapeMismatch { expected: vec![rows, cols], got: vec![slice.len()] }`
- [ ] Audit T02 (gemm.rs), T03 (distance.rs), T04 (normalize.rs/topk.rs): replace any
  inline guard logic with calls to these helpers; no duplication of the guard pattern
- [ ] Add `remediation: String` field to `ForgeError::NumericalInvariant` and
...
- [ ] The `CALYX_FORGE_NUMERICAL_INVARIANT` code must appear verbatim (as the
...
- [ ] unit: `check_finite(&[1.0, f32::NAN, 2.0], "test")` → `Err(ForgeError::NumericalInvariant)`;
  error `Display` contains `"CALYX_FORGE_NUMERICAL_INVARIANT"` at position 0
- [ ] unit: `check_finite(&[1.0, f32::INFINITY, 2.0], "test")` → `Err`; index 1 in detail
- [ ] unit: `check_norm_positive(0.0, "normalize", 7)` → `Err`; detail contains `"row 7"`
- [ ] proptest: any `ForgeError` produced by any CPU kernel with a NaN-containing
  input → `Display` starts with `"CALYX_FORGE_"` and contains `"Remediation:"`
- [ ] edge (≥3): (1) all-finite slice → `Ok(())`; (2) empty slice → `Ok(())`;
  (3) `f32::NEG_INFINITY` detected same as `INFINITY`
- [ ] fail-closed: `check_shape_2d` with mismatched sizes → `ForgeError::ShapeMismatch`;
...
- [ ] CPU↔GPU bit-parity ≤ 1e-3 on the golden set (guards do not alter numeric path)
- [ ] FSV evidence (readback output showing `CALYX_FORGE_NUMERICAL_INVARIANT`) attached to PH12 GitHub issue
