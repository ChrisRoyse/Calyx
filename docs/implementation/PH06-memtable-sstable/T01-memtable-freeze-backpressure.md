...
- [ ] Add `FrozenMemtable` as a newtype over the sorted entries (`BTreeMap` or
  `Vec<(Vec<u8>, Vec<u8>)>`) with methods `iter()`, `len()`, `flush_to_sst()`.
- [ ] Add `Memtable::freeze(self) -> FrozenMemtable` that consumes the memtable
  and returns an immutable view; no new writes are possible after freeze.
- [ ] Add `Memtable::needs_flush(&self) -> bool` returning `estimated_bytes >=
  byte_cap * 9 / 10` (at 90% capacity, trigger pre-flush).
- [ ] Keep the existing `put`, `get`, `iter`, `estimated_bytes`, `byte_cap`,
  `len`, `is_empty` API surface unchanged.
- [ ] Update `flush_to_sst` on `FrozenMemtable` to use the same `sst::write_sst`
  path.
- [ ] Write proptest: for any sequence of `(key: Vec<u8>, value: Vec<u8>)` pairs
  with `key.len() + value.len() <= byte_cap / 4`, after inserting until first
  `Err(CALYX_BACKPRESSURE)`, assert `estimated_bytes <= byte_cap`.
- [ ] Write proptest: `freeze()` followed by `iter()` returns the same keys in
  the same order as `Memtable::iter()` before freeze.

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: 3 puts that fit; freeze; frozen.len() == 3; frozen.flush_to_sst rounds
  trip byte-exact.
- [ ] unit: `needs_flush` triggers at 90% capacity: put until exactly 9/10 of
  cap; assert `needs_flush() == true`; put one byte over → `Err(CALYX_BACKPRESSURE)`.
- [ ] proptest: `∀ sequence of (k,v) pairs`: `estimated_bytes` after any sequence
  of successful puts ≤ `byte_cap`.
- [ ] edge (≥3): (1) empty freeze → `FrozenMemtable` with 0 entries; (2) same key
  inserted twice, second overwrites — byte estimate updates correctly; (3)
  `byte_cap = 0` → first put returns `CALYX_BACKPRESSURE`.
- [ ] fail-closed: overflow → `error.code == "CALYX_BACKPRESSURE"`; existing
  entries unaffected.
