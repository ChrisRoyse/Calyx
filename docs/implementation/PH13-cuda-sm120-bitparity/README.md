...
(RTX 5090, Blackwell GB202, CUDA 13.2) implementing the same `Backend` trait
defined in PH12. Deliver cuBLASLt-backed GEMM, fused cosine/topk kernels via
...
`calyx-forge` has the `Backend` trait, `CpuBackend`, error types, and golden
fixtures from PH12. The CUDA backend is entirely greenfield. Build natively on
aiwonder (`source ~/.cargo/env && cargo build`) with `CUDA_PATH=/usr/local/cuda-13.2`
and `RUSTFLAGS="-C target-cpu=native"` set in `env.sh`. No cross-build. sm_120
requires CUDA 13.2 (`nvcc -arch=sm_120`); older CUDA versions produce
`unsupported GPU architecture` errors — pin explicitly.
...
| `src/cuda/gemm.rs` | cuBLASLt GEMM wrapper (f32, bf16); sm_120 handle init; 10%-of-cuBLAS perf gate |
...
| T03 | cuBLASLt GEMM wrapper + 10%-of-cuBLAS perf gate | T02 |
...
Run on aiwonder (RTX 5090, sm_120, CUDA 13.2):
...
- **sm_120 / CUDA 13.2 only on aiwonder:** never run GPU tests on Windows dev
...
- **cudarc API surface:** cudarc 0.12.x broke several APIs; pin the exact version in
  ...
  the most recent that compiles against CUDA 13.2 headers).
