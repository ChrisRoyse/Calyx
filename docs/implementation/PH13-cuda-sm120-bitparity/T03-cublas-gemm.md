...
- [ ] `src/cuda/gemm.rs`: `pub fn gemm_cublas(ctx: &CudaContext, a: &CudaSlice<f32>, b: &CudaSlice<f32>, m: usize, k: usize, n: usize, out: &mut CudaSlice<f32>) -> Result<(), ForgeError>`
  — uses `cudarc::cublas::CublasHandle`; call `cublasSgemm_v2` with column-major
  layout (`CUBLAS_OP_N`, `CUBLAS_OP_N`), alpha=1.0, beta=0.0
- [ ] Handle allocation: `out` is pre-allocated by caller; no `cudaMalloc` inside
  the function (arena allocator contract from `13 §5`)
- [ ] On `cublasSgemm` error → `ForgeError::NumericalInvariant { op: "gemm_cublas", detail: "<cublasStatus_t name>", remediation: "..." }`
- [ ] `impl Backend for CudaBackend`: `gemm()` copies f32 slices to device via
...
- [ ] `pub fn bench_gemm_cublas(ctx: &CudaContext, m: usize, k: usize, n: usize, iters: u32) -> f64`
...
- [ ] `pub fn bench_gemm_reference_cublas(ctx: &CudaContext, m: usize, k: usize, n: usize, iters: u32) -> f64`
...
- [ ] unit `#[cfg(feature="cuda")]`: GEMM identity `A × I = A` for A=4×4 random
  f32; result on GPU matches CPU within 1e-4 element-wise
- [ ] unit: `bench_gemm_cublas` for 512×512×512, 10 iters → returns a positive
  finite GFLOP/s value (smoke test; no assertion on speed)
- [ ] `perf_vs_cublas` test: `forge_gflops / cublas_gflops >= 0.90` for m=k=n=512;
  if < 0.90 → test FAILS with message `"Forge GEMM ratio={ratio:.3} < 0.90 on sm_120"`
- [ ] proptest: random f32 matrices A (m×k) and B (k×n), m,k,n ∈ {1..32} →
  GPU result within 1e-3 rel of CPU result from PH12 `CpuBackend.gemm()`
- [ ] edge (≥3): (1) m=1 (row vector × matrix); (2) n=1 (matrix × column vector);
  (3) k=1 (outer product)
- [ ] fail-closed: GPU OOM simulated by requesting a 32 GB allocation → `ForgeError::DeviceUnavailable`
...
- [ ] FSV evidence (perf ratio + test log / screenshot) attached to PH13 GitHub issue
