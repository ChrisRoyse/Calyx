...
- [ ] `pub const MXFP4_BLOCK_SIZE: usize = 32;` — never a magic number in any other file
- [ ] `pub struct MxFp4Block { pub codes: [u8; 16], pub scale_e8m0: u8 }`
...
- [ ] `pub fn encode_mxfp4_block(block: &[f32; 32]) -> MxFp4Block`
...
- [ ] `pub fn decode_mxfp4_block(block: &MxFp4Block) -> [f32; 32]`
  — unpack nibbles; `code 15` → return 0.0 (NaN code); `x = (code - 7) as f32 * scale`
- [ ] `pub fn encode_mxfp4(vec: &[f32]) -> Result<Vec<MxFp4Block>, ForgeError>`
...
- [ ] sm version gate: `encode_mxfp4` is CPU-side packing only; the actual tensor-core
  GEMM dispatch (T02) checks `sm >= 12.0` at runtime

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: `encode_mxfp4_block([1.0; 32])` → `scale_e8m0 = 127` (2^0 = 1.0);
  all codes = 14 (value 7, the max); decode back → `[1.0; 32]` exactly
- [ ] unit: `encode_mxfp4_block([0.0; 32])` → `scale_e8m0 = 0` (or handled as zero-block);
  all codes = 7 (zero value); decode back → `[0.0; 32]`
- [ ] proptest: `max |decode(encode(v))[i] - v[i]| ≤ 2 * scale` (quantization error
  ≤ 1 LSB on each element) for random f32 vectors with all-positive values
- [ ] proptest: encode→decode preserves sign: `sign(decode(encode(v))[i]) == sign(v[i])`
  for non-zero elements
- [ ] edge (≥3): (1) block with one outlier value (rest near zero) — scale captures outlier;
  (2) `vec.len() = 31` (padding); (3) block with all-equal values
- [ ] fail-closed: block containing `f32::NAN` → `ForgeError::NumericalInvariant`
...
- [ ] CPU↔GPU bit-parity ≤ 1e-3 on the golden set (fp4 decode on CPU matches GPU decode)
