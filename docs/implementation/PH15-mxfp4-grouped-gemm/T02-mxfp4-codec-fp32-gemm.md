...
- [ ] `src/quant/mxfp4_codec.rs`: `pub struct MxFp4Codec { dim: usize }`
...
- [ ] CUDA GEMM path extension in `src/cuda/gemm.rs`: `pub fn gemm_mxfp4_fp32_accum(ctx: &CudaContext, a_blocks: &[MxFp4Block], b_blocks: &[MxFp4Block], m: usize, k: usize, n: usize, out: &mut CudaSlice<f32>) -> Result<(), ForgeError>`
...
- [ ] Document in `gemm_mxfp4_fp32_accum`: if CUDA 13.2 does not yet expose fp4-native
...
- [ ] unit: `MxFp4Codec::new(128).encode(&vec)` → `QuantizedVec` with `level == Bits4Fp`
- [ ] unit: encode→decode roundtrip for a dim-128 unit vector: `cosine(decoded, original) ≥ 0.95`
- [ ] unit: `gemm_mxfp4_fp32_accum` result for identity inputs agrees with f32 gemm
  within 5% (fp4 quantization error budget)
- [ ] proptest: encode→decode preserves vector sign for random unit vectors (same as T01)
- [ ] edge (≥3): (1) sm < 12.0 path → `DeviceUnavailable`; (2) `assay_safety_check_placeholder`
  returns false → `QuantError`; (3) `dim=1536` encode → no panic
- [ ] fail-closed: `gemm_mxfp4_fp32_accum` on non-sm120 device → `CALYX_FORGE_DEVICE_UNAVAILABLE`
...
- [ ] CPU↔GPU bit-parity ≤ 1e-3 on the golden set (fp4 GEMM vs f32 GEMM within 5% for fp4-safe slots)
