...
- [ ] `CfRouter::flush_cf(&mut self, cf) -> Result<SstSummary>`: freeze the CF's
  ...
  empty one.
- [ ] `CfRouter::get(&self, cf, key) -> Result<Option<Vec<u8>>>`: check active
  memtable first, then `SstLevel::get`.
- [ ] `CfRouter::range(&self, cf, start, end) -> Result<Vec<SstEntry>>`: merge
  active memtable entries in range with `SstLevel::range`; deduplicate (memtable
  wins); return sorted.
- [ ] SST file naming: `<seq:020>.sst` where seq is the next monotonic counter per
  CF (start at 1, persist nowhere — count existing files on open + 1).
- [ ] Re-export `CfRouter` from `cf/mod.rs`.

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: open router in tempdir; `put(Base, b"k1", b"v1")`; `get(Base, b"k1")`
  returns `Some(b"v1")`; no SST file yet (below flush threshold).
- [ ] unit: fill memtable to capacity; `put` triggers flush; SST file appears in
  `vault_dir/cf/base/`; `get` still returns correct values from SST.
- [ ] unit: write k1 in SST and k2 only in memtable; `range(b"", b"\xff")` returns
  both in sorted order.
- [ ] edge (≥3): (1) router opened on existing vault with SST files → reads them
  back; (2) two CFs written independently → each CF dir contains only its own SST;
  (3) `get` on absent key returns Ok(None).
- [ ] fail-closed: corrupt SST in CF dir → `get` returns `CALYX_ASTER_CORRUPT_SHARD`.
