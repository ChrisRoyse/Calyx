...
- [ ] `pub fn check_dim(actual: &SlotVector, expected: SlotShape) -> Result<()>`:
...
- [ ] `pub fn check_shape_variant(actual: &SlotVector, expected: SlotShape) -> Result<()>`:
  separate check for Dense vs Sparse vs Multi variant mismatch (same error
  code, different remediation suffix).
- [ ] Hook `check_dim` into `Registry::measure` and `Registry::measure_batch`
  as a post-call guard (wraps every runtime call uniformly).
- [ ] Stub check for `SlotVector::Multi { token_dim }` vs
  `SlotShape::Multi { token_dim }` for completeness even though no multi-vector
  runtime ships yet.

## Tests (synthetic, deterministic — known input → known bytes/number)

- [ ] unit: `check_dim(Dense { dim: 768 }, SlotShape::Dense(768))` → `Ok(())`.
- [ ] unit: `check_dim(Dense { dim: 128 }, SlotShape::Dense(768))` → `Err`
  with code `"CALYX_LENS_DIM_MISMATCH"`.
- [ ] unit: `check_dim(Sparse { ambient_dim: 30522 }, SlotShape::Dense(768))`
  → `Err` with code `"CALYX_LENS_DIM_MISMATCH"` (variant mismatch).
- [ ] integration: mock runtime declaring `Dense(4)` but returning `Dense(8)`
  → `Registry::measure` returns `CALYX_LENS_DIM_MISMATCH`; no vector returned.
- [ ] edge (≥3): (1) dim 0 declared, dim 0 returned → `Ok(())` (degenerate
  but not a mismatch); (2) `Multi { token_dim: 64 }` matches → `Ok(())`; (3)
  `Multi { token_dim: 64 }` vs `Multi { token_dim: 32 }` → mismatch.
- [ ] fail-closed: any mismatch → exact code `"CALYX_LENS_DIM_MISMATCH"` with
